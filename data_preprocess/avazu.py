...
def create_avazu_dataset(path, read_part=True, samples_num=5000, embed_dim=8):
    sparse_features = ['day', 'hour', 'id', 'C1', 'banner_pos', 'site_id', 'site_domain',
    ...
    train_data['day'], train_data['hour'] = train_data['hour'].map(lambda x: int(x[4:6])), \
                                            train_data['hour'].map(lambda x: int(x[6:8]))  # int强转去掉字符串前的0
    # sparse features
    le = LabelEncoder()
