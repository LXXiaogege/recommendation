...
def create_amazon_electronic_dataset(file, embed_dim, maxlen):
    ...
    for user_id, hist in tqdm(reviews_df.groupby('user_id')):
        ...
        for i in range(1, len(pos_list)):
            ...
            fake_hist.append([neg_list[i - 1], cate_list[neg_list[i - 1]]])
            fake_hist_i = fake_hist.copy()
            if i == len(pos_list) - 1:
                test_data.append([hist_i, fake_hist_i, [pos_list[i], cate_list[pos_list[i]]], 1])
                test_data.append([hist_i, fake_hist_i, [neg_list[i], cate_list[neg_list[i]]], 0])
            elif i == len(pos_list) - 2:
                val_data.append([hist_i, fake_hist_i, [pos_list[i], cate_list[pos_list[i]]], 1])
                val_data.append([hist_i, fake_hist_i, [neg_list[i], cate_list[neg_list[i]]], 0])
            else:
                train_data.append([hist_i, fake_hist_i, [pos_list[i], cate_list[pos_list[i]]], 1])
                train_data.append([hist_i, fake_hist_i, [neg_list[i], cate_list[neg_list[i]]], 0])
    ...
    """
    6. 得到新的训练集、验证集、测试集，格式为：'hist','faker_hist', 'target_item', 'label'；
    """
    # create dataframe,list convert to dataFrame
    train = pd.DataFrame(train_data, columns=['hist', 'fake_hist', 'target_item', 'label'])
    val = pd.DataFrame(val_data, columns=['hist', 'fake_hist', 'target_item', 'label'])
    test = pd.DataFrame(test_data, columns=['hist', 'fake_hist', 'target_item', 'label'])

    # if no dense or sparse features, can fill with 0
    """
    7. 由于序列的长度各不相同，因此需要使用tf.keras.preprocessing.sequence.pad_sequences方法进行填充；
    与DIN模型输入不同的地方是多了个negative sample sequence, **_hist_real_len
    """
    train_hist_real_len = [len(l) for l in train['hist']]
    val_hist_real_len = [len(l) for l in val['hist']]
    test_hist_real_len = [len(l) for l in test['hist']]

    print('==================Padding===================')
    train_X = [np.array([0.] * len(train)), np.array([0] * len(train)),
               pad_sequences(train['hist'], maxlen=maxlen),
               pad_sequences(train['fake_hist'], maxlen=maxlen),
               np.array(train['target_item'].tolist()),
               np.array(train_hist_real_len)]
    train_y = train['label'].values
    val_X = [np.array([0] * len(val)), np.array([0] * len(val)),
             pad_sequences(val['hist'], maxlen=maxlen),
             pad_sequences(val['fake_hist'], maxlen=maxlen),
             np.array(val['target_item'].tolist()),
             np.array(val_hist_real_len)]
    val_y = val['label'].values
    test_X = [np.array([0] * len(test)), np.array([0] * len(test)),
              pad_sequences(test['hist'], maxlen=maxlen),
              pad_sequences(test['fake_hist'], maxlen=maxlen),
              np.array(test['target_item'].tolist()),
              np.array(test_hist_real_len)]
